...
import { fetchCustomers, updateCustomer, deleteCustomer, toggleCustomerStatus } from '../../../../store/customersSlice';

export default function CustomersManagementTable() {
  const dispatch = useDispatch();
  const [customers, error] = useSelector((state) => [state.customers.list, state.customers.error]);
  ...
  if (error) {
    return <Typography color="error" variant="h6" align="center">Failed to load customers: {error}</Typography>;
  }
  ...
  const handleChangePassword = (id) => {
    dispatch(updateCustomer({ id, password: newPassword }))
      .unwrap()
      .then(() => {
  ...
  const handleConfirmAction = () => {
    if (confirmationAction === 'delete') {
      ...
      dispatch(toggleCustomerStatus(selectedCustomer.id));
    }
      ...
      <Paper sx={{ padding: 3, marginBottom: 3 }}>
        ...
        >
          ...
          <MenuItem onClick={() => handleOpenConfirmation('status')}>
            Change Status to {selectedCustomer?.status === 'active' ? 'Blocked' : 'Active'}
          </MenuItem>
        </Menu>
      </Paper>
      ...
      <Modal open={passwordModalOpen} onClose={() => setPasswordModalOpen(false)}>
        ...
        >
          ...
          <Button
            variant="contained"
            color="primary"
            onClick={() => handleChangePassword(selectedCustomer.id)}
            fullWidth
          >
            Update Password
          </Button>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="confirmation-dialog-description">
            Are you sure you want to {confirmationAction === 'delete' ? 'delete this customer' : 'change the status'}?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
